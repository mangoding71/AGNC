...
class Attacker(ch.nn.Module):
    ...
    def forward(self, adj_sys, A, target, eps, step_size, iterations, b_xent,
        ...
        def get_adv_examples(adj, A, return_a=False):
            ...
            # Keep track of the "best" (worst-case) loss and its corresponding input
            best_loss = None
            best_delta_A = None
            ...
            for _ in iterator:
                ...
                loss = Calculate_loss(self.model, adj, self.features, self.nb_nodes, b_xent, self.batch_size, self.sparse)
                ...
                if step.use_grad:
                    if est_grad is None:
                        ...
                        f = lambda _x: m * Calculate_loss(self.model, _x, self.features, self.nb_nodes, b_xent, self.batch_size, self.sparse)
            ...
            loss = Calculate_loss(self.model, self.preprocess(step.to_image(delta_A)+A), self.features,
        ...
        def get_adv_x_examples(x):
            ...
            for _ in iterator:
                ...
                loss = Calculate_loss(self.model, adj_sys, x, self.nb_nodes, b_xent, self.batch_size, self.sparse)
                # loss = mi_loss_neg(self.model, x, self.sp_adj_ori, self.features, self.nb_nodes, b_xent, self.batch_size, self.sparse)
                if self.show_attack:
                    print("     Attack Loss: {}".format(loss.detach().cpu().numpy()))
                ...
                if stepx.use_grad:
                    if est_grad is None:
                        ...
                        f = lambda _x: m * Calculate_loss(self.model, adj_sys, x, self.nb_nodes, b_xent, self.batch_size, self.sparse)
            ...
            loss = Calculate_loss(self.model, adj_sys, x, self.nb_nodes,
